...
def select(ctx: Context, args_list: list):
    ...
    parser = argparse.ArgumentParser(description='')
    parser.add_argument("--ctf-id", help="List ctf")
    parser.add_argument("--challenge-id", help="List challenges in selected ctf")
    args = parser.parse_args(args_list)

    ctf_id = args.ctf_id
    challenge_id = args.challenge_id

    if challenge_id is None and ctf_id is None:
        ctx.send("Bad arguments: please give either a ctf or challenge id")
        sys.exit()
    if challenge_id is not None and ctx.selected_ctf is None:
        ctx.send("Bad arguments: please select a ctf first")
        sys.exit()
    if challenge_id is not None:
        select_challenge(ctx, int(challenge_id))
    elif ctf_id is not None:
        select_ctf(ctx, int(ctf_id))
...
def flag(ctx: Context, flag: Optional[str]= None):
    not_implemented()
